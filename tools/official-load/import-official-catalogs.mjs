...
async function fetchText(url) {
  const res = await fetch(url, {
    headers: {
      'user-agent': 'ITINERA official catalog loader/0.17 (+official sources only)',
...
async function main() {
  ...
  for (const source of queue || []) {
    try {
      ...
      await supabase.from('itinera_official_import_queue').update({ last_attempt_at: new Date().toISOString(), last_status: `ok ${fetched.status}; studies=${studies.length}` }).eq('id', source.id);
      report.sources.push({ source_id: source.source_id, url: source.url, status: fetched.status, studies: studies.length, ok: true });
    } catch (error) {
      await supabase.from('itinera_official_import_queue').update({ last_attempt_at: new Date().toISOString(), last_status: `error: ${String(error.message || error).slice(0, 300)}` }).eq('id', source.id);
      report.sources.push({ source_id: source.source_id, url: source.url, ok: false, error: String(error.message || error) });
    }
  }
...
main()
  .then(report => {
    ...
    if (report.sources?.some(s => !s.ok)) process.exitCode = 2;
  })
  .catch(error => {
    console.error(error);
    process.exit(1);
