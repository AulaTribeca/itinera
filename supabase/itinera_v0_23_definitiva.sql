...
create temp table if not exists tmp_itinera_university_clean as
select
  case
    when lower(coalesce(level,'')) like '%grado%' then
      'grado-clean-' || md5(concat_ws('|', coalesce(university,''), coalesce(branch,''), coalesce(credits,''), coalesce(university_type,'')))
    when lower(coalesce(level,'')) like '%máster%' or lower(coalesce(level,'')) like '%master%' then
      'master-clean-' || md5(concat_ws('|', coalesce(university,''), coalesce(branch,''), coalesce(credits,''), coalesce(university_type,'')))
    else
      'univ-clean-' || md5(concat_ws('|', coalesce(level,''), coalesce(university,''), coalesce(branch,''), coalesce(credits,''), coalesce(university_type,'')))
  end as id,
  concat_ws(' · ', nullif(level,''), nullif(university,''), nullif(branch,''), case when nullif(credits,'') is not null then credits || ' ECTS' end) as title,
  level,
  university,
  coalesce(university_type,'') as university_type,
  ''::text as center,
  ''::text as province,
  ''::text as community,
  coalesce(branch,'Rama no especificada') as branch,
  ''::text as field,
  coalesce(credits,'') as credits,
  ''::text as modality,
  ''::text as cut_off_note,
  null::integer as places,
  coalesce(source_id,'educabase') as source_id,
...
where coalesce(university,'') <> ''
  and lower(coalesce(university,'')) not in ('total','universidad','no consta','sin especificar')
  and coalesce(level,'') <> ''
group by level, university, coalesce(university_type,''), coalesce(branch,'Rama no especificada'), coalesce(credits,''), coalesce(source_id,'educabase');
