...
-- Supabase/PostgreSQL: unaccent is not immutable by default.
-- Generated columns and trigram expression indexes require immutable expressions,
-- so ITINERA wraps unaccent in an immutable helper.
create or replace function public.itinera_immutable_unaccent(value text)
returns text
language sql
immutable
parallel safe
...
create table if not exists public.itinera_studies (
  id text primary key,
  name text not null,
  normalized_name text generated always as (lower(public.itinera_immutable_unaccent(name))) stored,
  type text not null,
  level text not null,
  family text,
  branch text,
...
create table if not exists public.itinera_study_aliases (
  ...
  alias text not null,
  normalized_alias text generated always as (lower(public.itinera_immutable_unaccent(alias))) stored,
...
create index if not exists itinera_documents_content_trgm on public.itinera_official_documents using gin (lower(public.itinera_immutable_unaccent(coalesce(content,''))) gin_trgm_ops);
...
language sql stable as $$
  with query as (
    select lower(public.itinera_immutable_unaccent(coalesce(q,''))) as nq
  ),
  aliases as (
