...
async function loadDataset() {
  try {
    ...
    // Fallback local si Netlify Blobs no está disponible o aún no hay caché.
  }
  return seed;
}
...
export default async (request) => {
  try {
    ...
    const results = (data.studies || [])
      .map((item) => {
        const haystack = [item.name, item.family, ...(item.keywords || [])].join(' ');
        const exact = q && normalise(haystack).includes(q);
        const fuzzy = q
          ? Math.max(...[item.name, item.family, ...(item.keywords || [])].map((x) => similarity(q, x)))
          : 0.55;

        let score = exact ? 1.25 : fuzzy;
        if (type !== 'all' && item.type !== type) score -= 0.6;
        if (family !== 'all' && normalise(item.family) !== normalise(family)) score -= 0.45;
        if (!q && (type !== 'all' || family !== 'all')) score += 0.25;

        return {
          ...item,
          score,
          suggestion: Boolean(q && !normalise(item.name).includes(q) && score > 0.45)
        };
      })
